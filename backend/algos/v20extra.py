...
def _coerce_scalar(value):
    if isinstance(value, pd.Series):
        return value.iloc[0] if not value.empty else float('nan')
...
def _run_v20_extra_for_symbol(symbol, period="6mo", interval="1d", target_increase=20, auto_adjusted=True):
    ...
    data["SMA_200"] = close_series.rolling(window=200).mean()
    ...
    while i < n:
        green_value = _coerce_scalar(data["Green"].iloc[i])
        if bool(green_value):
            start_idx = i
            while i < n and bool(_coerce_scalar(data["Green"].iloc[i])):
                i += 1
            end_idx = i - 1
            ...
            current_close = _coerce_scalar(present_close.iloc[-1])
            ...
            if perc_increase >= target_increase:
                ...
                lp_timestamp = pd.Timestamp(LP_date)
                if lp_timestamp in data.index:
                    LP_SMA_value = _coerce_scalar(data.loc[lp_timestamp, "SMA_200"])
                else:
                    nearest_idx = data.index.get_indexer([lp_timestamp], method='nearest')[0]
                    LP_SMA_value = _coerce_scalar(data["SMA_200"].iloc[nearest_idx])

                if pd.isna(LP_SMA_value):
                    continue

                print("SMA_200 on LP_Date:", LP_SMA_value)

                if float(LP) < float(LP_SMA_value):
...
def v20_extra_algo(symbol, period="6mo", interval="1d", target_increase=20, auto_adjusted=True):
    """
    Run the v20 extra algorithm for a single symbol (existing behaviour).
    """
    if isinstance(symbol, (list, tuple, set)):
        raise TypeError("Pass a single symbol string to v20_extra_algo. Use v20_extra_algo_threaded for multiple symbols.")
    return _run_v20_extra_for_symbol(symbol, period, interval, target_increase, auto_adjusted)


def v20_extra_algo_threaded(symbols, period="6mo", interval="1d", target_increase=20, auto_adjusted=True, max_workers=None):
    """
    Run the v20 extra algorithm for multiple symbols concurrently.

    Args:
        symbols: Iterable of symbol strings.
        period, interval, target_increase, auto_adjusted: Same as v20_extra_algo.
        max_workers: Optional explicit thread count.

    Returns:
        List of dictionaries: [{'symbol': 'ABC.NS', 'results': [...]}]
        Each 'results' entry mirrors the tuple structure returned by v20_extra_algo.
    """
    ...
    with ThreadPoolExecutor(max_workers=worker_count) as executor:
        future_map = {
            symbol: executor.submit(_run_v20_extra_for_symbol, symbol, period, interval, target_increase, auto_adjusted)
